...
-- Fix the part where the error likely occurred during sorting
-- Added stronger nil checks and logic for when timestamps are nil
function PvPAssistant.MatchHistory:SortMatches(matchHistories)
    table.sort(matchHistories, function(a, b)
        -- Ensure both a and b exist
        if not a or not b then
            return false
        end

        -- Handle nil timestamps explicitly
        if not a.timestamp then
            return false -- If a has no timestamp, it goes after b
        end
        if not b.timestamp then
            return true -- If b has no timestamp, a goes before b
        end

        -- Compare the timestamps if both are non-nil
